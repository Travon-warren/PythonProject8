...
class Zombie(pygame.sprite.Sprite):
    ...
    def __init__(self, platform_group, portal_group, min_speed, max_speed, window_width, window_height, fps):
        ...
        if gender == 0:
            ...
            for i in range(1, 11):
                self.walk_right_sprites.append(pygame.transform.scale(pygame.image.load(
                    f"assets/images/zombie/boy/walk/Walk ({i}).png"), (64, 64)))
            ...
            for i in range(1, 11):
                self.die_right_sprites.append(pygame.transform.scale(pygame.image.load(
                    f"assets/images/zombie/boy/dead/Dead ({i}).png"), (64, 64)))
            ...
            for i in range(1, 11):
                self.walk_right_sprites.append(pygame.transform.scale(pygame.image.load(
                    f"assets/images/zombie/girl/walk/Walk ({i}).png"), (64, 64)))
            ...
            for i in range(1, 11):
                self.die_right_sprites.append(pygame.transform.scale(pygame.image.load(
                    f"assets/images/zombie/girl/dead/Dead ({i}).png"), (64, 64)))
        ...
        self.image = self.walk_left_sprites[self.current_sprite] if self.direction == -1 else self.walk_right_sprites[self.current_sprite]
    ...
    def move(self):
        """Move the zombie"""
        if not self.is_dead:
            self.animate(self.walk_left_sprites if self.direction == -1 else self.walk_right_sprites, 0.5)
    ...
    def check_animations(self):
        ...
        if self.animate_death:
            self.animate(self.die_right_sprites if self.direction == 1 else self.die_left_sprites, 0.095)

        if self.animate_rise:
            self.animate(self.rise_right_sprites if self.direction == 1 else self.rise_left_sprites, 0.095)
