...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, platform_group, portal_group, bullet_group, window_width, window_height):
        ...
        for i in range(1, 11):
            self.move_right_sprites.append(
                pygame.transform.scale(pygame.image.load(f"./assets/images/player/run/Run ({i}).png"), (64, 64)))
        ...
        for i in range(1, 11):
            self.idle_right_sprites.append(
                pygame.transform.scale(pygame.image.load(f"./assets/images/player/idle/Idle ({i}).png"), (64, 64)))
        ...
        for i in range(1, 11):
            self.jump_right_sprites.append(
                pygame.transform.scale(pygame.image.load(f"./assets/images/player/jump/Jump ({i}).png"), (64, 64)))
        ...
        for i in range(1, 11):
            self.attack_right_sprites.append(
                pygame.transform.scale(pygame.image.load(f"./assets/images/player/attack/Attack ({i}).png"), (64, 64)))
    ...
    def reset(self):
        ...
        # Update the rectangle's bottom-left position
        self.rect.bottomleft = self.position

    def animate(self, sprite_list, speed):
        """Animate the player's actions."""
        # Check if the current sprite index is within bounds
        if self.current_sprite < len(sprite_list) - 1:
            # Increment the sprite index by the given speed
            self.current_sprite += speed
        else:
            # Reset to the first sprite
            self.current_sprite = 0
            # Reset jump animation flag if active
            if self.animate_jump:
                self.animate_jump = False
            # Reset fire animation flag if active
            if self.animate_fire:
                self.animate_fire = False
        # Update the player's image to the current sprite
